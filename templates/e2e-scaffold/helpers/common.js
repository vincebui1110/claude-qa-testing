/**
 * Common Helpers — Shared across all Shopify apps
 * Generic Polaris UI helpers (toast, spinner, tabs, modals, search, API wait)
 */
...
export async function waitForPageLoad(context) {
  await context.locator('.Polaris-Spinner, .Polaris-SkeletonPage')
...
export async function bulkAction(context, rowIndexes, actionName) {
  const checkboxes = context.locator('.Polaris-IndexTable-Checkbox, .Polaris-Checkbox');
  for (const idx of rowIndexes) {
    await checkboxes.nth(idx).click();
  }
  const bulkBtn = context.getByRole('button', {name: new RegExp(actionName, 'i')});
  await bulkBtn.click();
}
